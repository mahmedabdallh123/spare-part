...
def upload_image_to_github(image_file, image_id):
    try:
        ...
        try:
            ...
            repo.update_file(path=github_path, message=f"تحديث صورة {filename}", content=content, sha=contents.sha, branch=APP_CONFIG["BRANCH"])
        except GithubException as e:
            if e.status == 404:
                repo.create_file(path=github_path, message=f"إضافة صورة {filename}", content=content, branch=APP_CONFIG["BRANCH"])
            else:
                raise e
...
def display_image(image_url, width=200):
...
def display_images_grid(image_urls_string, cols=3, width=150):
    ...
    for i in range(0, len(urls), cols):
        row_cols = st.columns(min(cols, len(urls)-i))
        for j, col in enumerate(row_cols):
            if i+j < len(urls):
                with col:
                    display_image(urls[i+j], width=width)
    ...
    for idx, row in df.iterrows():
        with st.expander(f"📋 السجل #{idx+1} - التاريخ: {row.get('التاريخ', '')} - الماكينة: {row.get('المعدة', '')}"):
            ...
            if "الصور" in row and row["الصور"] and row["الصور"] != "":
                ...
                display_images_grid(row["الصور"], cols=3, width=200)
            ...
            if st.session_state.get(f"editing_{unique_id}_{idx}", False):
                st.markdown("---")
                st.markdown("### ✏️ تعديل البيانات والصور")
                
                # تحضير قائمة الصور الحالية
                current_images = []
                if "الصور" in row and row["الصور"] and row["الصور"] != "":
                    current_images = [url.strip() for url in row["الصور"].split(',') if url.strip()]
                
                # حالة حفظ الصور التي سيتم الاحتفاظ بها
                if f"keep_images_{unique_id}_{idx}" not in st.session_state:
                    st.session_state[f"keep_images_{unique_id}_{idx}"] = current_images.copy()
                ...
                with st.form(key=f"edit_form_{unique_id}_{idx}"):
                    ...
                    st.markdown("#### 🖼️ إدارة الصور")
                    # عرض الصور الحالية مع زر حذف لكل منها
                    if st.session_state[f"keep_images_{unique_id}_{idx}"]:
                        st.markdown("**الصور الحالية:**")
                        img_cols = st.columns(min(3, len(st.session_state[f"keep_images_{unique_id}_{idx}"])))
                        for i, img_url in enumerate(st.session_state[f"keep_images_{unique_id}_{idx}"]):
                            col_idx = i % 3
                            with img_cols[col_idx]:
                                display_image(img_url, width=150)
                                if st.button(f"🗑️ حذف", key=f"del_img_{unique_id}_{idx}_{i}"):
                                    st.session_state[f"keep_images_{unique_id}_{idx}"].pop(i)
                                    st.rerun()
                    else:
                        st.info("لا توجد صور حالياً")
                    
                    # إضافة صور جديدة
                    new_images = st.file_uploader("➕ إضافة صور جديدة (اختياري)", type=APP_CONFIG["ALLOWED_IMAGE_TYPES"], accept_multiple_files=True, key=f"new_imgs_{unique_id}_{idx}")
                    ...
                    if save_btn:
                        ...
                        new_image_urls = []
                        if new_images:
                            with st.spinner("جاري رفع الصور الجديدة..."):
                                for i, img in enumerate(new_images):
                                    if img.size <= APP_CONFIG["MAX_IMAGE_SIZE_MB"] * 1024 * 1024:
                                        img_id = f"{uuid.uuid4()}_{i}"
                                        url, err = upload_image_to_github(img, img_id)
                                        if url:
                                            new_image_urls.append(url)
                                        else:
                                            st.error(f"فشل رفع {img.name}: {err}")
                        
                        # دمج الصور القديمة (المحتفظ بها) والجديدة
                        final_images = st.session_state[f"keep_images_{unique_id}_{idx}"] + new_image_urls
                        final_images_str = ", ".join(final_images) if final_images else ""
                        ...
                        df.at[idx, "الصور"] = final_images_str
                        
                        sheets_edit[sheet_name] = df
                        
                        if save_and_push_to_github(sheets_edit, f"تعديل سجل في قسم {sheet_name}"):
                            st.cache_data.clear()
                            st.success("✅ تم تعديل البيانات والصور بنجاح!")
                            # تنظيف حالة الجلسة
                            del st.session_state[f"keep_images_{unique_id}_{idx}"]
                    ...
                    if cancel_btn:
                        # تنظيف وإلغاء
                        if f"keep_images_{unique_id}_{idx}" in st.session_state:
                            del st.session_state[f"keep_images_{unique_id}_{idx}"]
                        st.session_state[f"editing_{unique_id}_{idx}"] = False
                        st.rerun()
    
    # عرض جدول البيانات للقراءة السريعة
    display_df = df.copy()
                ...
                if search_term:
                    if search_columns is None:
                        ...
                        for col in df_filtered.columns:
                            try:
                                mask = mask | df_filtered[col].astype(str).str.contains(search_term, case=False, na=False, regex=False)
                            except:
                                continue
                        df_filtered = df_filtered[mask]
                    else:
        ...
        if results:
            ...
            for idx, row in combined_results.iterrows():
                with st.container():
                    ...
                    if "الصور" in row and row["الصور"] and row["الصور"] != "":
                        st.markdown("#### 🖼️ الصور المرفقة")
                        display_images_grid(row["الصور"], cols=3, width=150)
                    st.markdown("#### 📋 بيانات الصيانة")
                    col1, col2 = st.columns(2)
                    with col1:
                        st.markdown(f"**📅 التاريخ:** {row.get('التاريخ', '')}")
            ...
            with col_export1:
                ...
                st.download_button("📥 تحميل نتائج البحث كملف Excel", excel_file, f"search_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", key='download-excel', use_container_width=True)
            with col_export2:
                st.metric("عدد السجلات", len(combined_results))
        else:
            st.warning("⚠️ لا توجد نتائج مطابقة للبحث")
            st.info("💡 نصائح للبحث: تأكد من صحة كتابة كلمة البحث، جرب البحث بجزء من الكلمة، تأكد من اختيار القسم الصحيح")
...
def add_new_data_entry(sheets_edit, sheet_name):
    ...
    with st.form(key="add_data_form", clear_on_submit=False):
        ...
        uploaded_images = st.file_uploader("اختر الصور (يمكنك اختيار عدة صور)", type=APP_CONFIG["ALLOWED_IMAGE_TYPES"], accept_multiple_files=True, key=f"image_uploader_{sheet_name}")
        if uploaded_images:
...
def add_new_department(sheets_edit):
    ...
    st.info("سيتم إنشاء قسم جديد (شيت جديد) في ملف Excel")
    col1, col2 = st.columns(2)
    with col1:
        new_department_name = st.text_input("📝 اسم القسم الجديد:", key="new_department_name", placeholder="مثال: قسم الميكانيكا, قسم الكهرباء")
...
def add_new_machine(sheets_edit, sheet_name):
    ...
    df = sheets_edit[sheet_name]
    equipment_list = get_equipment_list_from_sheet(df)
    st.markdown("**الماكينات الموجودة حالياً:**")
    if equipment_list:
        for eq in equipment_list:
            st.markdown(f"- 🔹 {eq}")
    else:
        st.info("لا توجد ماكينات مسجلة بعد")
    st.markdown("---")
    new_machine = st.text_input("📝 اسم الماكينة الجديدة:", key=f"new_machine_{sheet_name}", placeholder="مثال: محرك رئيسي 1, مضخة مياه")
